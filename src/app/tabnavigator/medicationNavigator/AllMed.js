        ...
        <View style={styles.screenContainer}>
            ...
            }
            {
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <MedicationHandler
                            medDate={GetAppointmentDate(item.resource.hasOwnProperty('effectivePeriod') ? item.resource.effectivePeriod.start : null) + ' ' + GetMonthName(item.resource.hasOwnProperty('effectivePeriod') ? item.resource.effectivePeriod.start : null)}
                            medMonth={GetYear(item.resource.hasOwnProperty('effectivePeriod') ? item.resource.effectivePeriod.start : null)}
                            medicationDetails={capitalize(item.resource.medicationCodeableConcept.coding[0].display)}
                            directions={item.resource.dosage.text}
                            checking={item.resource.extension[1].valueString === "" ? ('(Not Available)') : (item.resource.extension[1].valueString)}
                            quantity={item.resource.dosage.dose.value + ' ' + item.resource.dosage.dose.unit}
                            presby={item.resource.extension[2].valueString}
                            pratname={item.resource.extension[3].valueString}
                        />
                    )}
